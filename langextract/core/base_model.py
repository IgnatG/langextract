...
class BaseLanguageModel(abc.ABC):
    ...
    async def async_infer(
        ...
        """Async language model inference with sync fallback.

        By default, delegates to the synchronous ``infer()`` method via
        ``asyncio.to_thread`` so that existing providers work without
        modification.  Providers that support native async (e.g. via
        ``litellm.acompletion``) should override this method for true
        non-blocking I/O.

        Args:
          batch_prompts: Batch of inputs for inference.
          **kwargs: Additional arguments for inference.

        Returns:
          List of Sequence of scored outputs, one per prompt.
        """
        return await asyncio.to_thread(
            lambda: list(self.infer(batch_prompts, **kwargs))
        )
