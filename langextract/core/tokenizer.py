...
# CJK Scripts that typically don't use spaces but should be separated from
# Latin characters.  Prevents "Hello世界" from being a single token.
                ...
                if current_type == g_type:
                    if current_type == TokenType.WORD:
                        ...
                        if last_grapheme == grapheme:
                            should_merge = True
                        elif len(last_grapheme) >= len(
                            grapheme
                        ) and last_grapheme.endswith(grapheme):
                            should_merge = True
