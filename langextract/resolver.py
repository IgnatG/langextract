        ...
        for group_index, group in enumerate(extraction_data):
            for extraction_class, extraction_value in group.items():
                ...
                if not isinstance(extraction_value, (str, int, float)):
                    logging.warning(
                        "Skipping extraction '%s' (group %d): extraction text must be a "
                        "string, integer, or float. Found: %s. This typically means the "
                        "LLM returned a nested object instead of a plain text value.",
                        extraction_class,
                        group_index,
                        type(extraction_value),
                    )
