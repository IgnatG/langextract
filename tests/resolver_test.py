...
class ExtractOrderedEntitiesTest(parameterized.TestCase):
    ...
    def test_extract_ordered_extractions_skips_dict_values(self):
        """Dict-typed extraction text is skipped with a warning, not raised."""
        resolver = resolver_lib.Resolver(
            format_type=data.FormatType.YAML,
        )
        test_input = [
            {
                "red_flag": {"title": "Bad clause", "description": "Details"},
            },
            {
                "obligation": "The client shall pay within 30 days.",
                "obligation_attributes": {"party": "Client"},
            },
        ]
        result = resolver.extract_ordered_extractions(test_input)
        # The dict-valued red_flag should be skipped, obligation kept
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].extraction_class, "obligation")
        self.assertEqual(
            result[0].extraction_text,
            "The client shall pay within 30 days.",
        )

    def test_extract_ordered_extractions_skips_list_values(self):
        """List-typed extraction text is also skipped gracefully."""
        resolver = resolver_lib.Resolver(
            format_type=data.FormatType.YAML,
        )
        test_input = [
        ...
        result = resolver.extract_ordered_extractions(test_input)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].extraction_class, "party")
        self.assertEqual(result[0].extraction_text, "Acme Corp")
...
class ResolverTest(parameterized.TestCase):
    _TWO_MEDICATIONS_JSON_UNDELIMITED = textwrap.dedent(
        f"""\
      {{
    ...
    _TWO_MEDICATIONS_YAML_UNDELIMITED = textwrap.dedent(
        f"""\
  {data.EXTRACTIONS_KEY}:
    - medication: "Naprosyn"
      medication_index: 4
